...
///
/// The scroll clock is read by the `HeroCollapseParallax` child, NOT by this body — so a scroll tick
/// re-renders only that tiny wrapper and never rebuilds the hero content (which, on the title hero, would
/// re-run the up-next episode scan). The wrapped content is built once by the parent and only has a
/// render-only `.offset` re-applied.
