...
final class TraktService {
    ...
    /// Bumped whenever a sync is started or invalidated (sign-out). `performRefresh` compares the
    /// generation it was started with before applying its snapshot, so a refresh that finishes after a
    ...
    func signOut() {
        pollTask?.cancel()
        pollTask = nil
        // Cancel and invalidate any in-flight library sync first: it would otherwise finish after the
        // clears below and re-assign all seven caches from the signed-out account's payloads.
        refreshTask?.cancel()
        refreshTask = nil
        refreshGeneration += 1
        persist(nil)
    ...
    func refreshLibrary(force: Bool = false) async {
        ...
        refreshTask = task
        await task.value
        // Only clear the slot if it's still ours — a sign-out during the await already cleared it, and
        // may have started nothing (or something newer) in its place.
        if refreshTask == task {
            refreshTask = nil
            lastRefreshAt = Date()
        }
    ...
    private func performRefresh(generation: Int) async {
        ...
        // The fetch and the off-actor build above are long awaits: bail out unless this sync is still
        // the current one and the account is still signed in, so a superseded or signed-out refresh
        // never writes its snapshot over cleared state.
        guard generation == refreshGeneration, isSignedIn else { return }
